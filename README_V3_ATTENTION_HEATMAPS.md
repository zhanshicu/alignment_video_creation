...
├── data/
│   ├── video_scene_cuts/              # Scene images (RGB)
│   │   └── {video_id}/
...
│       Columns: video_id, scene_number, keyword,
│                scene_image_path, keyword_mask_path,
│                attention_heatmap_path, filename
│
├── keyword_masks/                     # Keyword masks from CLIPSeg
│   └── {video_id}/
│       └── {video_id}-Scene-0xx-01.png
│
...
│
├── outputs/
│   ├── training_v3/                   # Model checkpoints
│   └── variants_v3/                   # Generated variants
│       ├── {video_id}/
│       │   ├── {variant_name}/
│       │   │   └── attention_heatmap/
│       │   │       └── {video_id}-Scene-0xx-01.jpg
│       │   ├── {variant_name}_scenes.csv
│       │   └── statistics.json
│       └── manifest.json
│
├── src/
│   ├── training/
│   │   └── dataset_v3.py              # NEW: Dataset with attention heatmaps
│   ├── video_editing/
│   │   └── experimental_variants_v3.py # NEW: Variant generator for heatmaps
│   └── data_preparation/
│       ├── control_tensor.py          # Control tensor builder
│       ├── keyword_heatmap.py         # CLIPSeg keyword segmentation
│       └── attention_heatmap.py       # Attention heatmap processing
│
├── data_validation.ipynb              # NEW: Validate data and create valid_scenes.csv
├── workflow_v3_with_attention_heatmaps.ipynb  # NEW: Complete workflow
└── README_V3_ATTENTION_HEATMAPS.md    # This file
```
...
- Control tensor: `[M_t, S_t]` where `S_t = M_t × attention_proportion`
...
keyword_mask:      keyword_masks/{video_id}/scene_1.png
attention_heatmap: attention_heatmap/{video_id}/{video_id}-Scene-001-01.jpg
```
...
2. **early_boost**: Boost attention on keyword in first 33% of scenes (×1.5)
3. **middle_boost**: Boost attention in middle 33% (×1.5)
4. **late_boost**: Boost attention in last 33% (×1.5)
5. **full_boost**: Boost attention in all scenes (×1.5)
6. **reduction**: Reduce attention in middle 33% (×0.5)
...
### 2. Keyword Masks
- **Path**: `keyword_masks/{video_id}/scene_{x}.png` (where x is scene number)
...
- **Example**: `keyword_masks/123456/scene_1.png`, `keyword_masks/123456/scene_2.png`

### 3. Attention Heatmaps
- **Path**: `attention_heatmap/{video_id}/{video_id}-Scene-0xx-01.jpg`
...
**Alignment Score**: `alignment_score = mean(M_t ⊙ A_t)`
...
3. **Manipulation type** (3 levels):
   - boost (×1.5), reduction (×0.5), none (control)
...
| early_boost   | First 33%       | Boost ×1.5   | Test early attention importance  |
| middle_boost  | Middle 33%      | Boost ×1.5   | Test middle attention importance |
| late_boost    | Last 33%        | Boost ×1.5   | Test late attention importance   |
| full_boost    | All scenes      | Boost ×1.5   | Test overall boost effect        |
| reduction     | Middle 33%      | Reduce ×0.5  | Test reduction effect            |
