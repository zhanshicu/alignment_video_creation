# Framework Update: V2 → V3 (Attention Heatmaps)
...
- **`src/training/dataset_v3.py`**
  ...
  - Computes alignment score as: `mean(keyword_mask ⊙ attention_heatmap)`
...
- **`workflow_v3_with_attention_heatmaps.ipynb`**
  ...
  - Steps: Data validation → Loading → Training → Variants → Inference
...
OLD (V2):
data/
├── alignment_score.csv        # Scalar alignment scores
├── keywords.csv
├── screenshots_tiktok/
└── keyword_masks/
...
NEW (V3):
data/
├── video_scene_cuts/          # RENAMED from screenshots_tiktok
│   └── {video_id}/
│       └── {video_id}-Scene-0xx-01.jpg  # SPECIFIC naming format
├── keywords.csv
├── valid_scenes.csv           # GENERATED by data_validation.ipynb
│
keyword_masks/                 # MOVED to root level
└── {video_id}/
    └── {video_id}-Scene-0xx-01.png  # SPECIFIC naming format
...
**Solution**:
- All will be resized to `image_size` (default 512×512)
